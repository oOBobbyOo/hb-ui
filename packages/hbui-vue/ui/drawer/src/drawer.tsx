...
export default defineComponent({
  ...
  inheritAttrs: false,
  props: drawerProps,
  emits: ['close', 'update:modelValue', 'open'] as string[],
  setup(props: DrawerProps, { emit, slots, attrs }) {
    const { overlayRef, drawerRef, drawerClasses, handleOverlayClick } = useDrawer(props, emit)

    return () => (
      <Teleport to='body'>
        {props.showOverlay && <DrawerOverlay ref={overlayRef} visible={props.modelValue} style={{ zIndex: props.zIndex - 1 }} onClick={handleOverlayClick} />}

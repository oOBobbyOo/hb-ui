  ...
  setup(props: AvatarProps) {
    ...
    const setDisplayName = (nameValue: string, widthValue: number): void => {
      ...
      if (customText.value) {
        ...
        getBackgroundColor(customText.value.substring(0, 1))
        return
      }
      // 根据name显示
      if (nameValue.length < 2) {
        ...
        if (/^[\u4e00-\u9fa5]/.test(nameValue)) {
          nameDisplay.value = nameValue.substring(nameValue.length - 2)
          // 英文开头
          ...
          if (/[_ -]/.test(nameValue)) {
            ...
            nameDisplay.value = strBefore.substring(0, 1).toUpperCase() + strAfter.substring(0, 1).toUpperCase()
          } else {
            // 一个英文名的情况显示前两个字母
            nameDisplay.value = nameValue.substring(0, 2).toUpperCase()
          }
        } else {
          // 非中英文开头默认取前两个字符
          nameDisplay.value = nameValue.substring(0, 2)
        }
      }
      if (widthValue < 30) {
        nameDisplay.value = nameValue.substring(0, 1).toUpperCase()
      }
      getBackgroundColor(nameValue.substring(0, 1))
    }
